...
`[REPO]` **203** Vitest tests; many exported operators lack dedicated tests (`matmul`, `bytes`, `withObjectAsync`, etc.).

`[REPO]` Golden harness: **23** case keys per profile (`scripts/golden/expected/key-sets.json`); includes `bool_int_eq`, `bool_int_add`, `bool_float_eq`, `bool_float_add`; key parity in `test/golden/key-parity.test.ts`; offline gate semantics in `pyrt-cases-version-gates.test.ts`. Vitest: **203** tests (`npm test`).
