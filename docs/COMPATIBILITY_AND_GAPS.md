...
- **Golden harness:** `scripts/golden/` (CPython reference vs pyrt). Run: `npm run golden` (CI matrix: Python 3.10, 3.12, 3.14). **25 case keys per profile version** (3.9–3.14): MRO/`isinstance`, rich compare, slice, contains, int↔float / int↔bool / bool↔float ops, sequence bool repetition (`seq_bool_mul`, `seq_bool_rmul`), descriptor precedence, class hooks (`init_subclass_called`, `set_name_called`), plus version-gated `match_args` (3.10+), buffer (3.12+), `annotate_x` (3.14+). Key parity: `npm run golden:keys`, `test/golden/key-parity.test.ts`.
