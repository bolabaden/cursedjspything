...
- **Cross-type ordering helper:** `helpers/cross-type-ordering.ts` — shared `lt`/`le`/`gt`/`ge` bidirectional `TypeError` registration (plans 452–468); consumers listed in §8.15 prose above. **`richcmp-incomparable.test.ts`** is intentionally not on this helper.
