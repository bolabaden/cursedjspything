...
Complex ordering is unsupported; **`hash`** raises unhashable **`TypeError`** (plan 900; `complex-hash.test.ts`); empty-spec **`__format__`** (plan 900). Full file index: §8.15 operator cross-type bullets and `docs/knowledgebase/50-execution/validation-ladder.md` rows (plan 920 audit; §8.15 sync in plan 921 / PR #589 when merged).
