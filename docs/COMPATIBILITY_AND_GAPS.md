...
- **Operator cross-type:** `operator-bool-float.test.ts`, `operator-bool-str-remaining-binary.test.ts`, `operator-bytes-conversion.test.ts`, `operator-bytes-cross-type.test.ts`, `operator-bytes-remaining-cross-type.test.ts`, `operator-bytes-scalar-cross-type.test.ts`, `operator-complex-cross-type-binary.test.ts`, `operator-complex-div-unary.test.ts`, `operator-complex-eq-pow.test.ts`, `operator-complex-inplace-cross-type.test.ts`, `operator-complex-pow-complex.test.ts`, `operator-complex-pow-floordiv.test.ts`, `operator-complex-scalar.test.ts`, `operator-container-cross-type.test.ts`, `operator-container-scalar-cross-type.test.ts`, `operator-float-str-remaining-binary.test.ts`, `operator-format-evidence.test.ts`, `operator-inplace-cross-type.test.ts`, `operator-int-bitwise-float.test.ts`, `operator-int-bool.test.ts`, `operator-int-float.test.ts`, `operator-int-shift.test.ts`, `operator-int-str-remaining-binary.test.ts`, `operator-matmul-evidence.test.ts`, `operator-numeric-conversion-evidence.test.ts`, `operator-pow-mod.test.ts`, `operator-rounding-evidence.test.ts`, `operator-scalar-complex-pow-edges.test.ts`, `operator-scalar-complex-pow-floordiv.test.ts`, `operator-scalar-complex-truediv-pow-modulo.test.ts`, `operator-str-bytes-cross-type.test.ts`, `operator-str-float.test.ts`, `operator-str-scalar.test.ts`, `operator-unary-evidence.test.ts`, `operator-zerodivision.test.ts`
- **Bytes operator evidence (plan 920 audit P2; plan 924):** canonical+remaining-binary pattern — **no merge** of `operator-bytes-cross-type` into `operator-bytes-remaining-cross-type` (happy-path file kept separate; fold deferred per audit):
  - **`operator-bytes-cross-type.test.ts`** — bytes+bytes `add` and bytes*int `mul` happy paths only (plan 418); cross-type rejects → remaining file
  - **`operator-bytes-remaining-cross-type.test.ts`** — bytes↔scalar binary rejects in both orders + bytes↔int/float/bool ordering (plans 412, 426); **not** bytes+bytes add
  - **`operator-bytes-scalar-cross-type.test.ts`** — bytes↔int/float/bool eq/ne; int*bytes mul repeat (plans 430, 707, 845); shared-tuple with cross-type for bytes*int only
  - **`operator-str-bytes-cross-type.test.ts`** — str↔bytes eq/ne, contains, ordering (plans 458/462/464)
  - **`operator-bytes-conversion.test.ts`** — `bytesProtocol()` conversion guards on str vs int/float `TypeError`
  - **`bytes-add.test.ts`** — bytes+bytes concatenation API evidence (plan 654); distinct from operator cross-type file
...
- **Scalar-complex pow evidence (plan 920 audit P3; plan 925):** int/float `**` complex is **shared-tuple** across three files — **no merge** of `pow-edges` or `truediv-pow-modulo` into `pow-floordiv` (fold deferred; high merge risk per audit):
  - **`operator-scalar-complex-pow-floordiv.test.ts`** — int/float `**` complex via `complex.__rpow__`; scalar `//`/`%` complex unsupported; `0**complex` `ZeroDivisionError` (plan 901)
  - **`operator-scalar-complex-pow-edges.test.ts`** — scalar `**` complex edge cases: bool subclass, `0**0j`, negative base, pure-imag exponent (plan 909)
  - **`operator-scalar-complex-truediv-pow-modulo.test.ts`** — scalar `/` complex via `complex.__rtruediv__`; `divmod` reject; three-arg `pow(..., mod)` with any complex operand → `ValueError: complex modulo` (plan 902)
  - **Complex-side pow homes (exponent typing):** int `**` only in **`operator-complex-eq-pow.test.ts`** (plan 897); float `**` in **`operator-complex-pow-floordiv.test.ts`** (plan 898); complex `**` complex in **`operator-complex-pow-complex.test.ts`** (plan 899)
...
- **`operator-scalar-complex-pow-edges.test.ts`** — scalar **`**`** complex edge cases (bool, **`0**0j`**, negative base; plan 909)
- **`operator-scalar-complex-truediv-pow-modulo.test.ts`** — scalar **`/`** complex via **`complex.__rtruediv__`**; divmod reject; three-arg **`pow(..., mod)`** with any complex operand → **`ValueError: complex modulo`** (plan 902)

Complex ordering is unsupported; **`hash`** raises unhashable **`TypeError`** (plan 900; `complex-hash.test.ts`); empty-spec **`__format__`** (plan 900). Full file index: §8.15 operator cross-type bullets and `docs/knowledgebase/50-execution/validation-ladder.md` rows (plan 920 audit; §8.15 sync in plan 921 (merged)).
...
**Evidence:** `test/cpython-derived/operator-str-scalar.test.ts`, `test/cpython-derived/bytes-getitem-compare.test.ts`, `test/cpython-derived/sequence-index-type.test.ts`, `test/cpython-derived/dict-keyerror.test.ts`, `test/cpython-derived/operator-zerodivision.test.ts`, `test/cpython-derived/operator-pow-mod.test.ts`, `test/cpython-derived/operator-int-shift.test.ts`, `test/cpython-derived/operator-int-bitwise-float.test.ts`, `test/core/method.test.ts` (pre-init `getMethodType` → **`PyRuntimeError`**, plan 460). **Golden harness:** `scripts/golden/errors.ts` — `GoldenHarnessError` / `KeyParityError` for harness failures (plan 472); not user-facing `Py*` types. **Remaining gap:** Vitest protocol mocks may still use plain JS `Error` for simulated `IndexError`; user-facing builtin slots and `getMethodType()` bootstrap use typed `Py*` exceptions where evidenced.
