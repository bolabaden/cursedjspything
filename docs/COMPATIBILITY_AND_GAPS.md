...
**Evidence:** `test/cpython-derived/operator-str-scalar.test.ts`, `bytes-getitem-compare.test.ts`, `sequence-index-type.test.ts`, `operator-zerodivision.test.ts`, `operator-pow-mod.test.ts`, `operator-int-shift.test.ts`. **Remaining gap:** other builtins and protocol fallbacks may still throw plain `Error` where CPython normalizes exception types.
