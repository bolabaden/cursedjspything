...
- **Bytes operator evidence (plan 920 audit P2; plan 924):** canonical+remaining-binary pattern — **`operator-bytes-cross-type` kept as dedicated happy-path file** (fold into remaining deferred):
  - **`operator-bytes-cross-type.test.ts`** — bytes+bytes `+` and bytes*int `*` happy paths only (plan 418); **not** eq/ne or int*bytes (see scalar-cross-type)
  - **`operator-bytes-scalar-cross-type.test.ts`** — bytes↔int/float/bool eq/ne; int*bytes and bytes*int mul repeat (plans 430, 845); shared-tuple bytes*int with cross-type file
  - **`operator-bytes-remaining-cross-type.test.ts`** — bytes↔scalar binary rejects in both orders + bytes↔int/float/bool ordering (plans 412, 426)
  - **`operator-bytes-conversion.test.ts`** — `bytesProtocol()` on str vs int/float `TypeError`
  - **`operator-str-bytes-cross-type.test.ts`** — str↔bytes eq/ne, contains, ordering (binary rejects in bytes-remaining; plans 458/462/464)
