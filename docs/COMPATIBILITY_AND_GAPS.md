...
**Evidence:** `test/cpython-derived/str-encode.test.ts`, `bytes-decode.test.ts`, `bytes-join.test.ts`, `bytes-split.test.ts`, `bytes-rsplit.test.ts`, `bytes-startswith-endswith.test.ts`, `bytes-partition.test.ts`, `bytes-splitlines.test.ts`, `bytes-strip.test.ts`, `bytes-find.test.ts`, `bytes-index.test.ts`, `bytes-count.test.ts`, `bytes-replace.test.ts`, `bytes-upper-lower.test.ts`, `bytes-capitalize.test.ts`, `bytes-swapcase.test.ts`, `bytes-center.test.ts`, `bytes-ljust-rjust.test.ts`, `bytes-getitem-compare.test.ts`, `bytes-slice-index.test.ts`, `operator-bool-float.test.ts`, `operator-bool-str-binary.test.ts`, `operator-bool-str-remaining-binary.test.ts`, `operator-bytes-conversion.test.ts`, `operator-bytes-cross-type.test.ts`, `operator-float-str-binary.test.ts`, `operator-float-str-divmod-pow.test.ts`, `operator-float-str-floordiv-mod.test.ts`, `operator-float-str-remaining-binary.test.ts`, `operator-format-evidence.test.ts`, `operator-inplace-cross-type.test.ts`, `operator-int-bool.test.ts`, `operator-int-float.test.ts`, `operator-int-str-binary.test.ts`, `operator-int-str-divmod-pow.test.ts`, `operator-int-str-remaining-binary.test.ts`, `operator-matmul-evidence.test.ts`, `operator-numeric-conversion-evidence.test.ts`, `operator-rounding-evidence.test.ts`, `operator-sequence-cross-type-add.test.ts`, `operator-str-float.test.ts`, `operator-str-scalar.test.ts`, `operator-unary-evidence.test.ts`, `sequence-cross-type.test.ts`, `sequence-repeat-bool.test.ts`, and `sequence-repeat-nonint.test.ts` port thin matrices from CPython `Lib/test/test_operator.py` / rich-compare spirit. Golden keys: `int_float_*`, `bool_int_*`, `bool_float_*`. **Remaining gap:** bytes lacks full method surface beyond decode/join/split/rsplit/startswith/endswith/partition/splitlines/strip/find/rfind/index/rindex/count/replace/upper/lower/capitalize/swapcase/center/ljust/rjust/getitem; PEP 3118 buffer protocol not implemented; float↔str beyond covered binary slots are not coerced the way CPython does.
