...
- **Codec argument guards (plan 478):** `str.encode` / `bytes.decode` require `str` `encoding` and `errors` kwargs; unknown handler → `ValueError` — `str-encode.test.ts`, `bytes-decode.test.ts`.
- **bytes.hex / fromhex errors (plan 480):** `bytes.fromhex` arg type guard; `bytes.hex` sep length/type errors — `bytes-hex-fromhex.test.ts`.
